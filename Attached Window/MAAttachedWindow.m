...
#define MAATTACHEDWINDOW_SCALE_FACTOR (float)[[NSScreen mainScreen] userSpaceScaleFactor]
...
- (void)_updateGeometry
{
    ...
    float halfWidth = (float)contentRect.size.width / 2.0f;
    float halfHeight = (float)contentRect.size.height / 2.0f;
    switch (_side) {
        case MAPositionTopLeft:
...
- (MAWindowPosition)_bestSideForAutomaticPosition
{
    ...
    float halfWidth = (float)viewSize.width / 2.0f;
    float halfHeight = (float)viewSize.height / 2.0f;
...
- (NSBezierPath *)_backgroundPath
{
    ...
    NSRect contentArea = NSInsetRect(_viewFrame,
                                     ...
                                     -viewMargin * scaleFactor);
    float minX = (float)NSMinX(contentArea) * scaleFactor;
    float midX = (float)NSMidX(contentArea) * scaleFactor;
    float maxX = (float)NSMaxX(contentArea) * scaleFactor;
    float minY = (float)NSMinY(contentArea) * scaleFactor;
    float midY = (float)NSMidY(contentArea) * scaleFactor;
    float maxY = (float)NSMaxY(contentArea) * scaleFactor;
...
- (void)setArrowBaseWidth:(float)value {
    float maxWidth = ((float)MIN(_viewFrame.size.width, _viewFrame.size.height) + 
...
- (void)setCornerRadius:(float)value {
    float maxRadius = (((float)MIN(_viewFrame.size.width, _viewFrame.size.height) + 
